  ...
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" size="small" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="状态" prop="status">
        <el-select v-model="queryParams.status" placeholder="用途状态" clearable>
          <el-option
    ...
    <el-table v-loading="loading" :data="usageList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="用途ID" align="center" prop="usageId" />
      <el-table-column label="用途名称" align="center" prop="usageName" :show-overflow-tooltip="true" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="用途名称" prop="usageName">
          <el-input v-model="form.usageName" placeholder="请输入用途名称" />
        </el-form-item>
...
export default {
    ...
    return {
      ...
      rules: {
        usageName: [
          ...
          { min: 0, max: 50, message: '用途名称长度不能超过50个字符', trigger: 'blur' }
        ],
        orderNum: [
  ...
  methods: {
    ...
    handleUpdate(row) {
      this.reset();
      const usageId = row.usageId || this.ids[0]
    ...
    submitForm() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.usageId != undefined) {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除用途编号为"' + usageIds + '"的数据项？').then(function() {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('tile/usage/export', {
        ...this.queryParams
    ...
    handleStatusChange(row) {
      ...
      this.$modal.confirm('确认要"' + text + '""' + row.usageName + '"吗？').then(function() {
